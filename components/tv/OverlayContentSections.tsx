        ...
        return (
          ...
          >
            <div className="flex flex-col">
              <div className="lg:hidden flex flex-col gap-2.5 px-4 pb-5 pt-4 sm:px-6 md:px-10 text-center justify-center">
                <h2 className="text-lg font-black leading-tight sm:text-xl">{section.title}</h2>
                <p className="text-sm leading-relaxed text-(--text-muted-color)">{section.text}</p>
              </div>

              <div className="relative overflow-hidden aspect-video w-full rounded-3xl lg:aspect-21/9 lg:min-h-105">
                <Image
                  src={section.image}
                  alt={section.title}
                  fill
                ...
                >
                  <div
                    className={`max-w-105 space-y-4 ${textToneClass} ${shadowClass} ${textAlign}`}
                  >
                    <h2 className="text-xl font-black leading-tight sm:text-xl lg:text-2xl 4xl:text-3xl">
                      {section.title}
                    </h2>
                    <p
